...
/// Календарь с выбором полной даты.
public struct DateControl: Control {

    /// ControlProtocol
    public let type: FormType = .date
    ...
    public let min: DateElement?

    public let max: DateElement?
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard case .date = try container.decode(FormType.self, forKey: .type) else {
        ...
        let min = try container.decodeIfPresent(DateElement.self, forKey: .min)
        let max = try container.decodeIfPresent(DateElement.self, forKey: .max)
